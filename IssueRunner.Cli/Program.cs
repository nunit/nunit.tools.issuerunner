...
internal static class Program
{
    ...
    private static Command BuildRunCommand(IServiceProvider services)
    {
        ...
        var scopeOption = new Option<TestScope>(
            "--scope",
            () => TestScope.All,
            "Test scope: All (default), Regression (closed issues), Open (open issues)");
        ...
        runCommand.AddOption(testTypesOption);
        ...
        runCommand.SetHandler(async (context) =>
        {
            ...
            var testTypes = context.ParseResult.GetValueForOption(testTypesOption);
    ...
    private static Command BuildResetCommand(IServiceProvider services)
    {
        ...
        resetCommand.AddOption(verbosityOption);

        resetCommand.SetHandler(async (issues, verbosity) =>
        {
            ...
            await cmd.ExecuteAsync(env.Root, issueNumbers, verbosity, CancellationToken.None);
        }, issuesOption, verbosityOption);
